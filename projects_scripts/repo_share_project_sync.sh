...
while IFS= read -r proj; do
    ...
    curl -XGET -H "Authorization: Bearer ${AUTH_TOKEN_JPD_1}" "${SOURCE_JPD_URL}/artifactory/api/repositories?project=$proj" -s | jq -r '.[].key' > "$proj.json"
    while IFS= read -r repo; do
      echo -e "Reponame name ==> $repo"
      echo -e "Share $repo to $proj in $TARGET_JPD_URL"
      curl --location --request PUT "$TARGET_JPD_URL/access/api/v1/projects/_/share/repositories/$repo/$proj" --header "Authorization: Bearer $AUTH_TOKEN_JPD_2"
    done < "$proj.json"
done < $projectfile

### sample cmd to run - ./repo_share_project_sync.sh https://ramkannan.jfrog.io http://35.209.109.173:8082 **** ****