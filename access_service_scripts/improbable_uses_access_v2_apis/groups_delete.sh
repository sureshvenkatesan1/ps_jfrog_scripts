...
### define variables
grouplist="groups_delete.txt"

### Run the curl API 
while IFS= read -r groupname; do
    echo -e "deleting group == $groupname =="
    curl -XDELETE -H "Authorization: Bearer $JPD_AUTH_TOKEN" "${JPD_URL}/access/api/v2/groups/${groupname}" -s
    echo -e ""
done < $grouplist

### sample cmd to run - ./groups_delete.sh https://ramkannan.jfrog.io admin ****