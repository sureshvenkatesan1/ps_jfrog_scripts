...
function display_file_extension_count() {
    ...
    echo
    grep -E ".*\.[a-zA-Z0-9]*$" "$1" | sed -e 's/.*\(\.[a-zA-Z0-9]*\)$/\1/' | sort | uniq -c | sort -n
}
...
#  extract the uri lines that are unique to the source.log file  that are not present in the target.log file
comm -23 <(sed -n '/uri/p' ${output_dir}/source.log | grep -v "https://" | sort) <(sed -n '/uri/p' ${output_dir}/target.log | grep -v "https://"| sort) > ${output_dir}/diff_output.txt
clean_and_format_paths ${output_dir}/diff_output.txt ${output_dir}/cleanpaths.txt
