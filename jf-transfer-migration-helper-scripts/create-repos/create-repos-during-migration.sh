...
### sample cmd to run - ./create-repos-during-migration.sh source-server target-server remote "acme-helm-bitnami-remote;ad-npm-remote" "ad-nuget-remote;alex-docker" "your_password"
...
password="${6:-}" # Optional password for remote repositories
...
for REPO in $(jf rt curl "api/repositories?type=${TYPE}" -s --server-id=$source | jq -r '.[].key'); do
    ...
    # Check if the repository is not in the inclusion list
        ...
        # Check if "rclass" is "remote" and set the password if provided
        if [[ -n "$password" ]]; then
            jq --arg password "$password" '. |= if .rclass == "remote" then .password = $password else . end' $REPO-config.json > temp.json
        else
