...
def create_remote_in_rt2(dest_url, dest_token, repo_name, repo_config, password, project_key):
    ...
    prefixed_repo_name = f"{project_key}-{repo_name}"
    url = f"{dest_url}/artifactory/api/repositories/{prefixed_repo_name}"
    headers = {
    ...
    repo_config['key'] = prefixed_repo_name

    response = requests.put(url, headers=headers, json=repo_config)
    
    if response.status_code in [200, 201]:
        logger.info(f"Successfully created remote repository {prefixed_repo_name} in Artifactory2")
        return True, prefixed_repo_name
    else:
        error_msg = f"Failed to create repository {prefixed_repo_name}: {response.status_code} - {response.text}"
        logger.error(error_msg)
        return False, None
